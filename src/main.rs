...
fn main() {
    ...
    store.set(b"test_key".to_vec(), b"initial_value".to_vec()).expect("Failed to set initial value");
    
    for i in 0..100 {
        let value = format!("value_{}", i).into_bytes();
        store.set(b"test_key".to_vec(), value).expect("Failed to update key");
    }
    
    let size_before = fs::metadata("db.log").expect("Failed to get metadata").len();
    println!("Log file size before compaction: {} bytes", size_before);
    
    store.compact().expect("Failed to compact");
    
    let size_after = fs::metadata("db.log").expect("Failed to get metadata").len();
    ...
    let final_store = kv::KVStore::open(Path::new("db.log")).expect("Failed to reopen after compaction");
    assert_eq!(final_store.len(), 1);
    assert_eq!(final_store.get(b"test_key" as &[u8]), Some(&b"value_99".to_vec()));
